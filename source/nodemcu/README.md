...
## Firmware HTTP (endpoint local)
Si necesitas abrir el porton via endpoint local (sin MQTT), usa:
- `firmware_http/firmware.ino`
Este firmware expone `/open?token=...` y `/status`.
